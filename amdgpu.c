...
int amd_temp_max, amd_power_hit_max, amd_power_max = 0;

long int amd_mem_used_max, amd_mem_total, amd_freq_max = 0;

void get_amdgpu_mem_usage() {
    FILE *fp;
...
void get_amdgpu_power() {
    FILE *fp;

    char power_buff[32],  amd_power_max_buff[32], power_path_buff[512], amd_power_max_path_buff[512];
    ...
    snprintf(amd_power_max_path_buff, 512, "%shwmon/%s/power1_cap_max", CARD0_PATH_AMDGPU, amd_hwmon);

    fp = fopen(amd_power_max_path_buff, "r");

    if (fp == NULL) {
        return;
    }

    fgets(amd_power_max_buff, 32, fp);
    fclose(fp);

    power = atoi(power_buff) / 1000 / 1000;
    amd_power_max = atoi(amd_power_max_buff) / 1000 / 1000;

    if (power > amd_power_hit_max) {
        amd_power_hit_max = power;
    }

    print_progress("GPU power", power, amd_power_max);
    printf(" (%dW / %dW)\n", power, amd_power_max);
}

void get_amdgpu_amd_power_hit_max() {
    print_progress("Max GPU power", amd_power_hit_max, amd_power_max);
    printf(" (%dW / %dW)\n", amd_power_hit_max, amd_power_max);
}

void get_amdgpu_temp() {
    FILE *fp;
...
void amdgpu_init() {
    ...
    get_amdgpu_mem_usage();
    get_amdgpu_mem_usage_max();
    printf("\n");
    get_amdgpu_power();
    get_amdgpu_amd_power_hit_max();
    printf("\n");
    get_amdgpu_freq();
    get_amdgpu_amd_freq_max();
    printf("\n");
    get_amdgpu_temp();
    get_amdgpu_amd_temp_max();
    printf("\n");
}
